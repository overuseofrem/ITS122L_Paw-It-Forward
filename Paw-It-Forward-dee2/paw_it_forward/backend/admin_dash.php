<?php
    ...
    // Automatically set PostStatus to 'Pending' if AmountRaised exceeds AmountNeeded
    $update_query = "UPDATE Post SET PostStatus = 'Pending' WHERE AmountRaised > AmountNeeded";
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            if (isset($_POST['approve'])) {
                // Count the number of already approved posts
                ...
                if ($approvedCount >= 3) {
                    header("Location: admin-dash.php?message=" . urlencode("You cannot approve more than 3 posts at a time. Please unapprove an existing post first."));
                    exit();
                }
        ...
        header("Location: ../admin-dash.php?message=" . urlencode($message));
        exit();
    }
